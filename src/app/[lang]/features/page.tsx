...
import { ExportRulesPreview } from "@/components/features/export-rules-preview";
import { AnalyticsPreview } from "@/components/features/analytics-preview";
import { AILetterPreview } from "@/components/features/ai-letter-preview";
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-16 md:gap-24 w-full">
        ...
        {/* Section 3: Export Calculation & Rules Engine */}
        <section className="flex flex-col gap-8 w-full pt-12 border-t border-border/40">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-border/40">
            <div className="max-w-3xl space-y-3">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold bg-amber-500/10 border border-amber-500/20 text-amber-400">
                ...
                <span>{t(f.rulesEngine.badge, locale)}</span>
              </div>
              <h2 className="text-2xl sm:text-3xl font-bold tracking-tight text-foreground">
                {t(f.rulesEngine.title, locale)}
              </h2>
              <p className="text-muted-foreground text-sm sm:text-base leading-relaxed">
                {t(f.rulesEngine.description, locale)}
              </p>
            </div>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {f.rulesEngine.highlights.map((h) => (
              <div
                key={h.id}
                className="p-4 rounded-xl bg-card/60 border border-border/60 hover:border-amber-500/30 transition-all flex flex-col gap-2"
              >
          ...
          <div className="w-full">
            <ExportRulesPreview locale={locale} />
          </div>
        </section>
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-border/40">
            <div className="max-w-3xl space-y-3">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold bg-emerald-500/10 border border-emerald-500/20 text-emerald-400">
                ...
                <span>{t(f.aiLetterGenerator.badge, locale)}</span>
              </div>
              <h2 className="text-2xl sm:text-3xl font-bold tracking-tight text-foreground">
                {t(f.aiLetterGenerator.title, locale)}
              </h2>
              <p className="text-muted-foreground text-sm sm:text-base leading-relaxed">
                {t(f.aiLetterGenerator.description, locale)}
              </p>
            </div>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {f.aiLetterGenerator.highlights.map((h) => (
              <div
                key={h.id}
                className="p-4 rounded-xl bg-card/60 border border-border/60 hover:border-emerald-500/30 transition-all flex flex-col gap-2"
              >
        ...
        <section className="flex flex-col gap-8 w-full pt-12 border-t border-border/40">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-border/40">
            <div className="max-w-3xl space-y-3">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold bg-cyan-500/10 border border-cyan-500/20 text-cyan-400">
                ...
                <span>{t(f.analytics.badge, locale)}</span>
              </div>
              <h2 className="text-2xl sm:text-3xl font-bold tracking-tight text-foreground">
                {t(f.analytics.title, locale)}
              </h2>
              <p className="text-muted-foreground text-sm sm:text-base leading-relaxed">
                {t(f.analytics.description, locale)}
              </p>
            </div>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {f.analytics.highlights.map((h) => (
              <div
                key={h.id}
                className="p-4 rounded-xl bg-card/60 border border-border/60 hover:border-cyan-500/30 transition-all flex flex-col gap-2"
              >
          ...
          <div className="w-full">
            <AnalyticsPreview locale={locale} />
          </div>
        </section>
        ...
        <section className="flex flex-col gap-8 w-full pt-12 border-t border-border/40">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-border/40">
            <div className="max-w-3xl space-y-3">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold bg-sky-500/10 border border-sky-500/20 text-sky-400">
                <Cloud className="w-3.5 h-3.5" />
                <span>{t(f.cloudStorage.badge, locale)}</span>
              </div>
              <h2 className="text-2xl sm:text-3xl font-bold tracking-tight text-foreground">
                {t(f.cloudStorage.title, locale)}
              </h2>
              <p className="text-muted-foreground text-sm sm:text-base leading-relaxed">
                {t(f.cloudStorage.description, locale)}
              </p>
            </div>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {f.cloudStorage.highlights.map((h) => (
              <div
                key={h.id}
                className="p-4 rounded-xl bg-card/60 border border-border/60 hover:border-sky-500/30 transition-all flex flex-col gap-2"
              >
          ...
          {/* <div className="w-full rounded-2xl border border-border/80 overflow-hidden shadow-2xl shadow-black/80"> */}
          {/* eslint-disable-next-line @next/next/no-img-element */}
          {/* <img
              src="/images/app-cloud-storage.png"
              alt="DocketLinks Cloud Storage — File management with organized folders for documents, exports, uploads, and user files"
              className="w-full h-auto"
            /> */}
          {/* </div> */}
        </section>

        {/* Section 7: Ecosystem & Upcoming Feature Cards */}
